...
func (reportRepository *ReportRepository) CreateReport(report datamodels.Report) (*mongo.InsertOneResult, error) {
...
func (ReportRepository *ReportRepository) GetAllReportsByProjectId(projectId string) ([]datamodels.Report, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (ReportRepository *ReportRepository) GetReportById(reportId string) datamodels.Report {
...
func (ReportRepository *ReportRepository) DeleteReportByID(reportId string) (interface{}, error) {
	...
	idPrimitive, err := primitive.ObjectIDFromHex(reportId)
	if err != nil {
...
func (ReportRepository *ReportRepository) UpdateReport(report datamodels.Report) (interface{}, error) {
	...
	update := bson.D{
		...
		{"$set", bson.D{{"url", report.Url}}},
		{"$set", bson.D{{"accessToken", report.AccessToken}}},
		{"$set", bson.D{{"headers", report.Headers}}},
		{"$set", bson.D{{"postParams", report.POSTParams}}},
		{"$set", bson.D{{"filterOptions", report.FilterOptions}}},
	}
